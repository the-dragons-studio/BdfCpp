...
BdfList::Iterator BdfList::find(BdfObject* needle) noexcept {
	// Search the entire BdfList until we find it.
	BdfList::Iterator first = this->begin();
	BdfList::Iterator last = this->end();
...
std::optional<uint64_t> BdfList::findIndex(BdfObject* needle) noexcept {
	// Search the entire BdfList until we find it.
	uint64_t itNo;
	BdfList::Iterator first = this->begin();
	BdfList::Iterator last = this->end();
...
std::partial_ordering BdfList::operator<=>(BdfList& rhs) noexcept {
	BdfList::Iterator lhsIt = this->begin();
	BdfList::Iterator rhsIt = rhs.begin();
...
bool BdfList::operator==(BdfList& rhs) noexcept {
